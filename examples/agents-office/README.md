...
|---|---|---|
| Реестр | `src/registry.js` | 24 агента: 15 своих из `agents/` и 9 из каталога репозитория |
| Каталог | `src/catalog.js` | 203 позиции с готовыми карточками на двух языках, поиск и индекс |
| Модель | `src/llm.js` | Вызов Claude, инструменты каталога, потоковая выдача текста |
...
24 агента в восьми отделах. Пятнадцать написаны под Vitaflow, девять взяты
...
| Поддержка | Support Responder, **Direct Responder**, **Review Collector**, Retail Customer Returns |
| Товар и деньги | **Margin Analyst**, **Restock Planner**, Pricing Analyst, Finance Tracker |
| Трафик | Growth Hacker |
...
- **Нет связи с ботом и заказами.** Каталог берётся из выгрузки; заказы
  и клиенты в систему не заведены. Следующий шаг — читать `/api/catalog`
  магазина напрямую и подключить заказы через SSH к SQLite в режиме `mode=ro`.
- **Нет расписания.** Задачи не запускаются сами по времени.
- **Нет отправки покупателю.** Агент готовит текст, отправляете вы. Это
  намеренно: автоотправка от имени магазина — слишком дорогая ошибка.
