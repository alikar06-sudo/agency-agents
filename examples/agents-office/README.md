...
- **Нет связи с ботом и заказами.** Каталог уже можно читать из магазина
  вживую (`AO_CATALOG_URL`), а вот заказы и клиенты в систему не заведены.
  Следующий шаг — подключить заказы через SQLite в режиме `mode=ro`.
